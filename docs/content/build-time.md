...
> Everything below was checked against `v5.0.0`, the version the examples pin. Quartz's `v5` branch has since moved its plugins to npm packages and deleted the lockfile, so a newer `quartz-version` will not behave as described here. That path has not been tested.

## The first build

Several minutes, usually around ten. Quartz at v5.0.0 ships no prebuilt distribution, so its plugins are cloned and compiled from source — 42 of them in the default set, roughly 2.5 GB on disk, even with a warm npm cache.

After that the action caches the compiled plugins, and later runs restore them in seconds rather than rebuilding. Each repository keeps its own cache.

There are two entries, which is why **Actions → Caches** shows a pair: the compiled plugins, and npm's own download cache (`~/.npm`) that feeds `npm ci`. The plugin one is the large one.

## What makes a build slow again

A new `quartz-version` — or `quartz-repository` — starts from nothing and pays the full cost again, since both are part of the cache key. So does the first run in a repository, and any run after the cache has been evicted: GitHub drops entries that have gone seven days without a read.
