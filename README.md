...
- Plugins are cached separately, keyed on the Quartz version plus a hash of the `source:` lines in your config — so a title or theme edit keeps the cache, and adding a plugin builds only that one. Removing a plugin leaves its files in the cache: nothing prunes them, and each run carries them into the next entry. They are inert; to clear them, bump `quartz-version` or delete the caches under Actions → Caches.
